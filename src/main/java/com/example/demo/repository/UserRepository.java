...
public interface UserRepository extends JpaRepository<User, Long> {
        ...
        COALESCE(u.Reputation, 0) AS voteScore
        FROM Users u LEFT JOIN User_Profile p ON u.user_id = p.user_id
        WHERE u.role != 'admin' AND u.role != 'bot' AND (:currentUserId IS NULL OR u.user_id <> :currentUserId) AND (:keyword IS NULL OR :keyword = '' OR u.username LIKE CONCAT('%', :keyword, '%'))
        ORDER BY CASE WHEN :filter = 'reputation' OR :filter = 'voted' THEN COALESCE(u.Reputation, 0) END DESC,
        CASE WHEN :filter = 'new' THEN u.created_at END DESC, u.username ASC
        """, countQuery = """
        SELECT COUNT(*) FROM Users u WHERE u.role != 'admin' AND u.role != 'bot' AND (:currentUserId IS NULL OR u.user_id <> :currentUserId) AND (:keyword IS NULL OR :keyword = '' OR u.username LIKE CONCAT('%', :keyword, '%'))
        """, nativeQuery = true)
